<template lang="pug">
  .recharge-select(v-if="isLoad")
    .recharge-select-top
      .recharge-select-options
        .select-options-left
          span 选择充值设备
        .select-options-right
          p(@click="toRechargeRecord") 充值记录
          img(src="./arrow_select.png")
      .recharge-select-tip
        p 友情提醒
        p 1.请根据医生提供的设备号为对应的设备进行充值
        p 2.充值时，充值设备需处于开机状态
    .recharge-select-list(v-for="(item,index) in in_hospital" :key="index")
      ...
      ul.recharge-list-content
        li.recharge-list-item(@click="toRechargeFrequency(items)", v-for="(items,indexs) in item.list", :key="indexs")
          .item-left
            .icon
    ...
    .recharge-select-list
      .recharge-list-header
        h3 未添加到医院的设备
      ul.recharge-list-content
        li.recharge-list-item(@click="toRechargeFrequency(item)", v-for="(item,index) in out_hospital" :key="index")
          .item-left
            .icon
              svg.ali_icon(aria-hidden="true")
                use(xlink:href="#iconicon_treatment_")
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toRechargeRecord(){
        this.$router.push(`/boss/recharge/record`)
      },
      toRechargeFrequency(items){
        this.STORE_DATA({
          rechargeState:'create',
          rechargeListItem: items
        })
        this.$router.push(`/boss/recharge/frequency`)
      }
    }
  }
</script>
